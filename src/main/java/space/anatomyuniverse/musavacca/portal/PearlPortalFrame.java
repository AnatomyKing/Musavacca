...
public final class PearlPortalFrame {
    ...
    public record Shape(
        ...
        public Shape withFrontDirection(Direction frontDirection) {
            return new Shape(this.axis, this.minCorner, this.width, this.height, frontDirection);
        }
        ...
        public Vec3 center() {
            if (this.axis == Direction.Axis.X) {
                return new Vec3(
                        this.minCorner.getX() + this.width / 2.0D,
                        this.minCorner.getY() + this.height / 2.0D,
                        this.minCorner.getZ() + 0.5D
                );
            }

            return new Vec3(
                    this.minCorner.getX() + 0.5D,
                    this.minCorner.getY() + this.height / 2.0D,
                    this.minCorner.getZ() + this.width / 2.0D
            );
        }
        ...
        public void forEachInteriorBlock(Consumer<BlockPos> consumer) {
            ...
            for (int x = 0; x < this.width; x++) {
                for (int y = 0; y < this.height; y++) {
                    consumer.accept(this.minCorner.relative(widthDir, x).above(y));
                }
            }
        }
    }
    ...
    public static Optional<Shape> findIgnitableShape(BlockGetter level, BlockPos insidePos) {
        ...
        return xShape.isPresent()
                ? xShape
                : findShape(level, insidePos, Direction.Axis.Z, PearlPortalFrame::canBecomePortalInterior);
    }
    ...
    public static boolean isValidFrontDirection(Direction.Axis axis, Direction direction) {
        return direction != null
                ...
                && direction.getAxis() == normalAxis(axis);
    }
    ...
    private static int measureWidth(
        ...
        while (width <= MAX_WIDTH) {
            if (!isInterior(level, minCorner.relative(widthDir, width), interiorPredicate)) {
                break;
            }
    ...
    private static int measureHeight(
        ...
        while (height <= MAX_HEIGHT) {
            if (!isInterior(level, minCorner.above(height), interiorPredicate)) {
                break;
            }
    ...
    private static boolean isValidFrame(
        ...
        for (int x = 0; x < shape.width(); x++) {
            for (int y = 0; y < shape.height(); y++) {
                if (!interiorPredicate.test(level.getBlockState(shape.minCorner().relative(widthDir, x).above(y)))) {
                    return false;
                }
            }
        }
