...
public final class CubeFireBlockTinted32 {
    private static final int FIRE_LAYER_COUNT = 32;

    //? if >=1.21.4 {
    private static final TextureSlot[] FIRE_SLOTS = createFireSlots();
    //?}

    private CubeFireBlockTinted32() {}
    ...
    /*public static void generate(BlockStateProvider gen, Map<Block, Entry> entries) {
        ...
        entries.forEach((block, entry) -> {
            ...
            ModelFile floor0 = fireModel(gen, ns, entry.modelStem() + "_floor0", entry.floorParent(), entry.textureStem(), 0);
            ModelFile floor1 = fireModel(gen, ns, entry.modelStem() + "_floor1", entry.floorParent(), entry.textureStem(), 1);

            ModelFile side0 = fireModel(gen, ns, entry.modelStem() + "_side0", entry.sideParent(), entry.textureStem(), 0);
            ModelFile side1 = fireModel(gen, ns, entry.modelStem() + "_side1", entry.sideParent(), entry.textureStem(), 1);

            ModelFile sideAlt0 = fireModel(gen, ns, entry.modelStem() + "_side_alt0", entry.sideAltParent(), entry.textureStem(), 0);
            ModelFile sideAlt1 = fireModel(gen, ns, entry.modelStem() + "_side_alt1", entry.sideAltParent(), entry.textureStem(), 1);

            ModelFile up0 = fireModel(gen, ns, entry.modelStem() + "_up0", entry.upParent(), entry.textureStem(), 0);
            ModelFile up1 = fireModel(gen, ns, entry.modelStem() + "_up1", entry.upParent(), entry.textureStem(), 1);

            ModelFile upAlt0 = fireModel(gen, ns, entry.modelStem() + "_up_alt0", entry.upAltParent(), entry.textureStem(), 0);
            ModelFile upAlt1 = fireModel(gen, ns, entry.modelStem() + "_up_alt1", entry.upAltParent(), entry.textureStem(), 1);
    ...
    private static ModelFile fireModel(
        ...
        var builder = gen.models().withExistingParent(name, ResourceLocation.parse(parentModelId));

        for (int layer = 0; layer < FIRE_LAYER_COUNT; ++layer) {
            builder.texture("fire_" + layer, fireTexture(namespace, textureStem, frame, layer));
        }
    ...
    *///?} else {
    public static void generate(BlockModelGenerators gen, Map<Block, Entry> entries) {
        ...
        entries.forEach((block, entry) -> {
            ...
            TextureMapping tex0 = fireTextureMapping(ns, entry.textureStem(), 0);
            TextureMapping tex1 = fireTextureMapping(ns, entry.textureStem(), 1);
    ...
    private static TextureMapping fireTextureMapping(String namespace, String textureStem, int frame) {
        TextureMapping mapping = new TextureMapping();

        for (int layer = 0; layer < FIRE_LAYER_COUNT; ++layer) {
            mapping.put(
                    FIRE_SLOTS[layer],
                    fireTexture(namespace, textureStem, frame, layer)
            );
        }
    ...
    private static TextureSlot[] createFireSlots() {
        TextureSlot[] slots = new TextureSlot[FIRE_LAYER_COUNT];
