...
public final class CubeVocoReceptor {
    ...
    /**
     * Explicit existing model ids for both VocoReceptor states.
     *
     * unlit -> LIT = false
     * lit   -> LIT = true
     *
    ...
    /*public static void generate(BlockStateProvider gen, Map<Block, Models> models) {
        ...
        models.forEach((block, stateModels) -> {
            ...
            gen.getVariantBuilder(block).forAllStates(state -> {
                boolean litState = state.getValue(VocoReceptorBlock.LIT);
        ...
        models.forEach((block, stateModels) -> {
            ...
            /*gen.blockStateOutput.accept(
                    MultiVariantGenerator.multiVariant(block).with(
                            PropertyDispatch.property(VocoReceptorBlock.LIT)
            ...
            *///?} else {
            ...
            multi = multi.with(
                    BlockModelGenerators.condition().term(VocoReceptorBlock.LIT, false),
