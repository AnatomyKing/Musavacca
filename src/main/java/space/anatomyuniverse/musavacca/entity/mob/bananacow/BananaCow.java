...
public class BananaCow extends Cow {
    ...
    private static final String TAG_PEEL_STAGE = "BananaCowPeelStage";
    private static final String TAG_EATEN_BITES = "BananaCowEatenBites";
    ...
    private static final float GOLDEN_CARROT_SATURATION_MODIFIER = 1.2F;

    private static final EntityDataAccessor<Integer> DATA_PEEL_STAGE =
            SynchedEntityData.defineId(BananaCow.class, EntityDataSerializers.INT);
    ...
    protected void defineSynchedData(@NotNull SynchedEntityData.Builder builder) {
        ...
        builder.define(DATA_PEEL_STAGE, PEEL_STAGE_DEFAULT);
        builder.define(DATA_EATEN_BITES, 0);
    }
    ...
    public void setPeelStage(int peelStage) {
        this.entityData.set(DATA_PEEL_STAGE, clampInt(peelStage, PEEL_STAGE_DEFAULT, PEEL_STAGE_PEELD));
    }
    ...
    public void setEatenBites(int eatenBites) {
        this.entityData.set(DATA_EATEN_BITES, clampInt(eatenBites, 0, MAX_EATEN_BITES_BEFORE_DEATH));
    }
    ...
    public boolean isBananaPeeld() {
        return this.getPeelStage() >= PEEL_STAGE_PEELD;
    }

    public boolean canBananaCowBeShearedAgain() {
        return this.isAlive() && this.getPeelStage() < PEEL_STAGE_PEELD;
    }

    public boolean canBananaCowBeEaten() {
        return this.isAlive()
                && this.getPeelStage() >= PEEL_STAGE_SHEARED
                && this.getEatenBites() < MAX_EATEN_BITES_BEFORE_DEATH;
    }
    ...
    public @NotNull InteractionResult mobInteract(@NotNull Player player, @NotNull InteractionHand hand) {
        ...
        if (player.isShiftKeyDown() && this.canBananaCowBeEaten()) {
            if (!this.level().isClientSide && this.level() instanceof ServerLevel serverLevel) {
                this.eatBananaCow(serverLevel, player);
            }

            return InteractionResult.SUCCESS;
        }

        if (stack.is(Items.SHEARS) && this.canBananaCowBeShearedAgain()) {
            if (!this.level().isClientSide && this.level() instanceof ServerLevel serverLevel) {
                this.shearBananaCow(serverLevel, player, hand, stack);
            }
    ...
    private void shearBananaCow(@NotNull ServerLevel level,
                                @NotNull Player player,
                                @NotNull InteractionHand hand,
                                @NotNull ItemStack shears) {
        int nextStage = clampInt(this.getPeelStage() + 1, PEEL_STAGE_DEFAULT, PEEL_STAGE_PEELD);
        this.setPeelStage(nextStage);

        this.dropBananaPellis(level);

        this.playBananaCowSound(level, SoundEvents.SHEEP_SHEAR, 1.0F, 1.0F);

        player.swing(hand, true);

        if (!player.getAbilities().instabuild) {
            shears.hurtAndBreak(1, player, slotForHand(hand));
        }
    }

    private void dropBananaPellis(@NotNull ServerLevel level) {
        Containers.dropItemStack(
                level,
                this.getX(),
                this.getY() + 0.5D,
                this.getZ(),
                new ItemStack(ModItems.BANANA_PELLIS.get())
        );
    }

    private void eatBananaCow(@NotNull ServerLevel level, @NotNull Player player) {
        int nextBites = clampInt(this.getEatenBites() + 1, 0, MAX_EATEN_BITES_BEFORE_DEATH);
        ...
        if (nextBites >= MAX_EATEN_BITES_BEFORE_DEATH) {
            this.hurtBananaCow(level, source, FINAL_EAT_DAMAGE);
        } else {
            this.hurtBananaCow(level, source, EAT_DAMAGE);
        }
    }

    private void feedPlayerLikeGoldenCarrot(@NotNull Player player) {
    ...
    private void playBananaCowEatingFinishSounds(@NotNull ServerLevel level) {
        this.playBananaCowSound(level, SoundEvents.GENERIC_EAT, 1.0F, 1.0F);
        this.playBananaCowSound(level, SoundEvents.PLAYER_BURP, 0.55F, 1.15F);
    }

    private void playBananaCowSound(@NotNull ServerLevel level,
    ...
    private void hurtBananaCow(@NotNull ServerLevel level,
    ...
    protected void addAdditionalSaveData(@NotNull ValueOutput output) {
        ...
        output.putInt(TAG_EATEN_BITES, this.getEatenBites());
    }
