...
public final class VocoCameraClientModEvents {
    ...
    public static void register(IEventBus modBus) {
        ...
        //? if >=1.21.6 {
        modBus.addListener(
                VocoCameraClientModEvents::registerClientPayloads
        );
        //?}

        modBus.addListener(
                VocoCameraClientModEvents::registerGuiLayers
        );

        NeoForge.EVENT_BUS.addListener(
                VocoCameraClient::onClientTickPre
        );

        NeoForge.EVENT_BUS.addListener(
                VocoCameraClient::onClientTickPost
        );

        NeoForge.EVENT_BUS.addListener(
                VocoCameraClient::onMouseButton
        );

        NeoForge.EVENT_BUS.addListener(
                VocoCameraClient::onScreenOpening
        );

        NeoForge.EVENT_BUS.addListener(
                VocoCameraClient::onLogout
        );

        NeoForge.EVENT_BUS.addListener(
                VocoCameraClient::onRenderHand
        );
    }
